...
def _render_glade_md(
    ...
    packages_section = (
        "\n".join(f"- {n}" for n in sorted(notable_packages[:20]))
        if notable_packages
...
async def _run_init_async(
    bridge_url: str, force: bool, dry_run: bool
) -> int:
...
def run_init(
    bridge_url: str = DEFAULT_BRIDGE_URL, force: bool = False, dry_run: bool = False
) -> int:
